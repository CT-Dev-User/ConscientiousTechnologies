...
const SubPageRelibleTech = () => {
  ...
  const fetchReliableDataByCategory = async (subcategory) => {
    try {
      const response = await axios.get(
        `https://conscientious-technologies-backend.vercel.app/get-reliable-tools-data/${subcategory}`
      );
  ...
  useEffect(() => {
    fetchReliableDataByCategory(subcategory);
  }, [subcategory]);
  ...
  return (
    <div className="w-full bg-gray-300 h-full mx-auto p-4">
      <div className="flex justify-end mb-5 mr-3 gap-x-3">
        <Button
          onClick={() => navigate("/conscientious-reliable-tools")}
        ...
        >
          Add {subcategory} Reliable Tools Data
        </Button>
      </div>
      ...
      <ul className="flex justify-center mt-[90px]">
        ...
        {[
          ...Array(Math.ceil(reliableToolData.length / itemsPerPage)).keys(),
        ].map((number) => (
...
export default SubPageRelibleTech;
