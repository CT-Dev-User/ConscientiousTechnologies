...
const ContactUsForm = () => {
  ...
  const fetchContactUsData = async () => {
    try {
      const response = await axios.get("http://localhost:8080/get-contact-us-form-data");
  ...
  const deleteContactUsData = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(`https://conscientious-technologies-backend.vercel.app/delete-contact-form-us-data/${id}`);
