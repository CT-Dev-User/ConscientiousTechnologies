                ...
                <Modal.Body className="bg-white">
                    <form className="mx-auto max-w-lg">
                        <fieldset className="mb-4">
                            <label htmlFor="SubCategory" className="block text-gray-700 font-bold">SubCategory of {category}</label>
                            {dropdown.length !== 0 ?
                                <select className="mb-4 form-select" onChange={(e) => setAddData({ ...addData, SubCategory: e.target.value })}>
                ...
                <Modal.Body className="bg-white">
                    <form className="mx-auto max-w-lg">
                        <fieldset className="mb-4">
                            <label htmlFor="SubCategory" className="block text-gray-700 font-bold">SubCategory of {category}</label>
                            {dropdown.length !== 0 ?
                                <select className="mb-4 form-select" onChange={(e) => setEditData({ ...editData, SubCategory: e.target.value })}>
                                    <option>--select {category}--</option>
                                    {dropdown.map((item, index) => (
                                        <option key={index} value={item.title}>
                                            {item.title}
                        ...
                        {currentItems.map((item) => (
                            <tr key={item._id} className="border border-gray-300">
                                ...
                                <td className="py-2 border border-gray-300 flex justify-center space-x-2">
                                    ...
                                    <button className='hover:bg-red-700 h-[37px] bg-[green] px-[20px] py-[7x] rounded-[7px] text-white shadow-md' onClick={() => navigate(`/conscientious-area-of-experties-subpage/${item.SubCategory}`)}>SubPages</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
