  ...
  const fetchExperties = async () => {
    try {
      const response = await axios.get(
        `https://conscientious-technologies-backend.vercel.app/get-experies-data`
      );
  ...
  const addExpertiesDataFunc = async () => {
    try {
      ...
      const response = await axios.post(
        "https://conscientious-technologies-backend.vercel.app/add-experies-data",
        formData
      );
  ...
  const editExpertiesFunc = async () => {
    try {
      ...
      const response = await axios.put(
        `https://conscientious-technologies-backend.vercel.app/edit-experties-by-id/${editId}`,
        formData
      );
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.delete(
            `https://conscientious-technologies-backend.vercel.app/delete-experties-by-id/${id}`
          );
          if (response.status === 200) {
            fetchExperties();
  ...
  return (
    <div className="w-full bg-gray-300 h-full mx-auto p-4">
      <div className="flex justify-end mb-5 mr-3 gap-x-3">
        <Button
          onClick={() => navigate("/conscientious-area-of-experties")}
          className="bg-blue-100 hover:bg-blue-700 text-white font-bold py-1 px-2 rounded"
        >
          Back
        </Button>
        <Button
          ...
          <form className="mx-auto max-w-lg">
            ...
            <fieldset className="mb-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setAddData({ ...addData, SubCategory: e.target.value })
                }
              />
            </fieldset>
            <fieldset className="mb-4">
          ...
          <form className="mx-auto max-w-lg">
            ...
            <fieldset className="mb-4">
              ...
              <JoditEditor
                ...
                onChange={(value) =>
                  setEditData({ ...editData, subTitle: value })
                }
              />
            </fieldset>
            <fieldset className="mb-4">
              ...
              <tr key={item._id} className="border border-gray-300">
                ...
                <td className="py-2 border border-gray-300 text-center">
                  <img
                    src={item.image}
                    alt={item.title}
                    className="object-cover w-[70px] h-[70px] rounded-t-lg mx-auto"
                  />
                </td>
                <td className="py-2 border border-gray-300 flex justify-center space-x-2">
                  ...
                  <button
                    ...
                    onClick={() => deleteExpertiesData(item._id)}
                  >
                    Delete
                  </button>
                </td>
        ...
        <div className="mt-4 flex justify-center">
          {expertiesData.length > itemsPerPage && (
            <ul className="flex space-x-2">
              {Array.from({
                length: Math.ceil(expertiesData.length / itemsPerPage),
              }).map((_, index) => (
                <li key={index}>
                  <button
