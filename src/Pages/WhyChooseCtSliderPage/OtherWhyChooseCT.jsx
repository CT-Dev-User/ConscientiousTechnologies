...
const OtherWhyChooseCT = () => {
    ...
    const fetchSliderDataByCategory = async () => {
        try {
            ...
            setsliderDataByCaregory(filteredData);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const addSliderDataFunc = async (e) => {
        ...
        try {
            ...
            formData.append('category', addSliderData.category); // Ensure this is needed, otherwise remove it
    ...
    const editSliderDataFunc = async (e) => {
        ...
        try {
            const formData = new FormData();
            formData.append('category', editSliderData.category);
                ...
                <Modal.Body>
                    <form onSubmit={addSliderDataFunc} className="mx-auto max-w-lg">
                        <fieldset className="mb-4">
                            ...
                            <input type="text" className="form-control" id="category" name="category" value={addSliderData.category} onChange={(e) => { setAddSliderData({ ...addSliderData, category: e.target.value }) }} />
                        </fieldset>
                        <fieldset className="mb-4">
            ...
            <Modal show={editPopupShow} onHide={() => setEditPopUpShow(false)}>
                ...
                <Modal.Body>
                    <form onSubmit={editSliderDataFunc} className="mx-auto max-w-lg">
                        <fieldset className="mb-4">
                            <label className="block text-gray-700 font-bold">SubCategory</label>
                            <input type="text" className="form-control" id="category" name="category" value={editSliderData.category} onChange={(e) => handleEditSliderDataChange('category', e.target.value)} />
                        </fieldset>
                        <fieldset className="mb-4">
                            <label className="block text-gray-700 font-bold">SubCategory</label>
                ...
                <tbody>
                    {currentItems.map((slider, i) => (
                        <tr key={slider._id}>
                            ...
                            <td className="border p-2">{slider.category}</td>
...
export default OtherWhyChooseCT