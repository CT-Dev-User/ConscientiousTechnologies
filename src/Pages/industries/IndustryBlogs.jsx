...
const IndutryBlogCMS = () => {
  ...
  const currentItems = filterBlogs
    ? filterBlogs.slice(indexOfFirstItem, indexOfLastItem)
    : [];
  ...
  const fetchBlogs = async () => {
    ...
    try {
      const response = await axios.get(
        "https://conscientious-technologies-backend.vercel.app/get-latest-blog-data-by-category/Industry"
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentBlog) {
        ...
        // Create new blog
        await axios.post(
          "https://conscientious-technologies-backend.vercel.app/create-new-blog-data",
          form,
          {
            headers: { "Content-Type": "multipart/form-data" },
          }
            ...
            <select
              ...
              onChange={(e) => {
                if (e.target.value === "All Industries") {
                  setFilterBlogs(blogs); // Show all FAQs
                } else {
                  setFilterBlogs(
