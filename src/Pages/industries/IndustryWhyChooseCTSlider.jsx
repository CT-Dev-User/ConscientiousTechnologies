...
const IndustryWhyChooseCT = () => {
    ...
    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    const currentItems = filterwhyChooseCt ? filterwhyChooseCt.slice(indexOfFirstItem, indexOfLastItem) : [];
    ...
    const fetchSliderDataByCategory = async (subcategory) => {
        setLoading(true);
        try {
            const response = await axios.get(`https://conscientious-technologies-backend.vercel.app/get-choose-ct-slider-data/Industry`);
    ...
    const addSliderDataFunc = async (e) => {
        ...
        try {
            ...
            formData.append('category', "Industry"); // Ensure this is needed, otherwise remove it
                ...
                <div className='w-full h-full mx-auto p-4'>
                    <div className="flex justify-end mb-5 mr-3 gap-x-3">
                        <select
                            ...
                            onChange={(e) => {
                                if (e.target.value === "All Industries") {
                                    setFilterwhyChooseCt(sliderDataByCaregory); // Show all FAQs
                                } else {
                                    setFilterwhyChooseCt(
                                        sliderDataByCaregory.filter((item) => item.Subcategory === e.target.value)
                                    );
                                }
                            }}
                        ...
                        >
                            ...
                            {sliderDataByCaregory.map((industry, index) => (
                            ...
                            <form onSubmit={addSliderDataFunc} className="mx-auto max-w-lg">
                                <fieldset className="mb-4">
                                    ...
                                    <select
                                        ...
                                        onChange={(e) => { setAddSliderData({ ...addSliderData, Subcategory: e.target.value }) }}
...
export default IndustryWhyChooseCT;
