...
const ServiceWhyChooseCT = () => {
    ...
    const fetchSliderDataByCategory = async () => {
        try {
            const response = await axios.get(`https://conscientious-technologies-backend.vercel.app/get-choose-ct-slider-data/Service`);
                    ...
                    <div className="flex flex-wrap  gap-4">
                        {sliderImages.map((image, index) => (
                            <div className='w-[70px] border border-gray-300 p-2'>
                                <img key={index} src={image.logo} alt={`Logo ${index}`} className="w-full  h-auto" />
                            </div>
                        ))}
                    </div>
                </Modal.Body>
