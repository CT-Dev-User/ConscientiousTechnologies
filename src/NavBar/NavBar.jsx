  ...
  return (
    <nav className="shadow-lg shadow-gray-100 bg-slate-500 text-[white] w-[90%] h-[60px] mx-auto mt-[10px] px-[20px] box-border sticky top-[10px] mb-[20px]">
      <div className='py-[12px] mx-auto flex justify-between items-center'>
        ...
        {toggle ?
          <AiOutlineClose onClick={() => { setToggle(!toggle) }} className='text-white text-2xl hidden md:block' /> :
          <AiOutlineMenu onClick={() => { setToggle(!toggle) }} className='text-white text-2xl  hidden md:block ' />}
        <ul className='md:hidden flex text-white gap-10'>
          <li>User Name</li>
        </ul>
        {/* Responsive Menu */}
        <ul className={`duration-1000 hidden md:block w-full h-screen text-white fixed bg-black top-[92px]
